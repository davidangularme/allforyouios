...
@synthesize tabledviewController;
...
- (id)init {
    ...
    NSString *path = [documentsDirectory stringByAppendingPathComponent:@"datacounterallforyou.plist"]; NSFileManager *fileManager = [NSFileManager defaultManager];
    
    if (![fileManager fileExistsAtPath: path]) 
    {
        path = [documentsDirectory stringByAppendingPathComponent: [NSString stringWithFormat: @"datacounterallforyou.plist"] ];
    }
    
    
    
    
    NSFileManager *fileManager123 = [NSFileManager defaultManager];
    NSMutableDictionary *data;
    
    if ([fileManager123 fileExistsAtPath: path]) 
    {
    ...
    else
    {
        ...
        int tempvalue = 0;
        [data setObject:[NSNumber numberWithInt:tempvalue] forKey:@"counterallforyou"];
...
- (void)loadView {
 ...
 // lalalasql   
    ...
    sharedManager.flag = @"NO" ;
    while (object = [e nextObject]) {
 
        sharedManager.label1 = [object valueForKey:@"timeStamp"] ;
        sharedManager.label2 = [object valueForKey:@"labelmemo"] ;
        sharedManager.dataplist = [object valueForKey:@"plistname"] ; 
        
        NSLog(@"Found timeStamp %@", sharedManager.label1);
        NSLog(@"Found labelmemo %@", sharedManager.label2);
        NSLog(@"Found plistname %@", sharedManager.dataplist);
    ...
    sharedManager.flag = @"YES" ;
/*    
    MyManager *sharedManager = [MyManager sharedManager];
   if( sharedManager.flag == @"NO")
   {
    if ([self.exposeDelegate respondsToSelector:@selector(shouldAddViewControllerForExposeController:)]) {
        [self.exposeDelegate shouldAddViewControllerForExposeController:self];
    }
...
/*     
     ...
     [newManagedObject setValue:ppp forKey:@"timeStamp"];
...
*/     
     MyManager *sharedManager = [MyManager sharedManager];
     NSString *yuyu = sharedManager.someProperty;
     ...
     int value1,value;
     value1 = [[savedStock objectForKey:@"counterallforyou"] intValue];
     NSLog(@"%i",value1);
     
     value = value1 + 1;
     //To insert the data into the plist
    
     [data setObject:[NSNumber numberWithInt:value] forKey:@"counterallforyou"];
     [data writeToFile: path atomically:YES];
     ...
     ///////////////// essai plist ////////////////
     sharedManager.dataplist = [NSString stringWithFormat:@"%d", value];
      
     int val = value ;
     [self insertNewObject:yuyu labelmemo:ppp plistname:val];
...
/*     
     ...
     NSManagedObject* object;
     while (object = [e nextObject]) {
         NSLog(@"Found object %@", [object valueForKey:@"timeStamp"]);
     }
...
*/
     [containerViewtemp addSubview:labeltemp];
...
- (void)readfromdatabase
{
    ...
    NSManagedObject* object;
    while (object = [e nextObject]) {
        NSLog(@"Found timeStamp %@", [object valueForKey:@"timeStamp"]);
        NSLog(@"Found labelmemo %@", [object valueForKey:@"labelmemo"]);
        NSLog(@"Found plistname %@", [object valueForKey:@"plistname"]);
    ...
    /*     NSString* launchTitle = [NSString stringWithFormat:@"launch %d", [results count]];
     ...
     [object setValue:launchTitle forKey:@"timeStamp"];
...
- (void)newViewController:(UIViewController *)viewController index:(NSUInteger)index {
    ...
    MyManager *sharedManager = [MyManager sharedManager];
   NSString * condition = @"YES";
   if(sharedManager.flag == condition)
   {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Please"
                                                    message:@"Enter a name for this memo" 
                                                   delegate:self 
  ...
  //  [self.view addSubview:tableView];  
    ...
    if ([self.exposeDataSource respondsToSelector:@selector(exposeController:labelForViewController:)]) {
        ...
        label.frame = CGRectMake((containerView.exposeWidth - label.exposeWidth) / 8, containerView.exposeHeight + label.exposeTop, label.exposeWidth*6, label.exposeHeight);
        NSString *yyy = [[NSNumber numberWithInteger: index]stringValue ];
        label.text = [label.text stringByAppendingString:yyy] ;
        containerViewtemp = containerView;
        condition = @"NO";
        if(sharedManager.flag == condition)
        {
            label.text = sharedManager.label2 ;
        }
        else {
            labeltemp = label ;
        }
...
////////////////////////////////////////////
    ...
    NSEnumerator *e = [results objectEnumerator];
    NSManagedObject* object;
    for (int ii = 0; ii < deleteIndex; ii++) {
        object = [e nextObject];
    } 
    object = [e nextObject];
    NSLog(@"Found timeStamp %@", [object valueForKey:@"timeStamp"] );
...
- (void)toggleExpose {
   ...
   // [self.parentViewController exposeController];
}
...
- (void)insertNewObject:(NSString*) namememo  labelmemo:(NSString *)labelmemo plistname:(int32_t)plistname{
    ...
    // If appropriate, configure the new managed object.
    [newManagedObject setValue:namememo forKey:@"timeStamp"];
    [newManagedObject setValue:labelmemo forKey:@"labelmemo"];
    
    [newManagedObject setValue:[NSNumber numberWithInt:plistname] forKey:@"plistname"];
    
///////////////// essai plist ////////////////
/*    
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES); 
    NSString *documentsDirectory = [paths objectAtIndex:0]; 
    NSString *path = [documentsDirectory stringByAppendingPathComponent:@"1.plist"]; NSFileManager *fileManager = [NSFileManager defaultManager];
    
    if (![fileManager fileExistsAtPath: path]) 
    {
        path = [documentsDirectory stringByAppendingPathComponent: [NSString stringWithFormat: @"1.plist"] ];
    }
...
- (LIExposeController *)exposeController {
    return (LIExposeController *)objc_getAssociatedObject(self, (__bridge CFTypeRef)kExposeController);
}

- (void)setExposeController:(LIExposeController *)exposeController {
    objc_setAssociatedObject(self, (__bridge CFTypeRef)kExposeController, exposeController, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
